...
export function installTaskUI({service,getStaff,getAttendance,getCurrentStaff,document:doc=document,compress=compressTaskPhoto}) {
  ...
  const stats=rows=>{const s=taskTotals(rows);return `<div class="nt-stats"><div>کل کام<b>${s.total}</b></div><div>منظوری باقی<b>${s.pending}</b></div><div>منظور شدہ پوائنٹس<b>${s.points}</b></div></div>`};
  function cards(rows){return rows.map(t=>`<article class="nt-card"><span class="nt-state ${e(t.status)}">${e(STATUS[t.status]||t.status)}</span><h3>${e(t.title)}</h3><small>${e(t.staffName)} • ${e(t.date)}</small><p>${e(t.details)}</p>${t.ownerNote?`<p class="nt-note">مالک: ${e(t.ownerNote)}</p>`:''}<p><b>پوائنٹس: ${t.status==='approved'?Number(t.points):0} / ${Number(t.maxPoints)}</b> • ${t.photoIds?.length||0} تصاویر</p><button type="button" class="${session.role==='staff'&&['assigned','changes_requested'].includes(t.status)?'nt-primary':''}" data-task-open="${e(t.id)}">${session.role==='staff'&&['assigned','changes_requested'].includes(t.status)?'📷 کام / تصاویر جمع کریں':session.role==='owner'&&t.status==='submitted'?'تصاویر دیکھیں اور OK کریں':'تفصیل اور تصاویر'}</button></article>`).join('')||'<p class="nt-empty">ابھی کوئی کام نہیں۔</p>'}
  ...
  const api={
    start(next){api.stop();session={...next};const ticket=epoch;render();try{unsub=service.listen((rows,cached)=>{if(ticket!==epoch)return;tasks=rows;ready=true;fromCache=cached;loadError='';render()},err=>{if(ticket!==epoch)return;ready=false;tasks=[];loadError=err;render()})}catch(err){loadError=err;render()}},
    stop(){epoch++;unsub?.();unsub=null;session=null;tasks=[];ready=false;loadError='';closeDialog();closeReport();closeImage();for(const id of ['ntOwnerTaskList','ntStaffTaskList','ntOwnerTaskStats','ntStaffTaskStats','ownerProfileTasksV99'])if($(id))$(id).innerHTML=''},
    assign,open,report,
