import {createTask,submissionPatch,reviewPatch,validPhoto,requireValue} from './task-model.js';
...
export function createTaskService({fs,auth,sdk,getStaffPhone,businessId='noor-traders'}) {
  ...
  return {
    ...
    listen(next,error) {
      ...
      return onSnapshot(ref,{includeMetadataChanges:true},s=>{try{unchanged(a);const list=[];s.forEach(d=>list.push({...d.data(),id:d.id}));next(list,!!s.metadata?.fromCache)}catch(e){error(e)}},error);
    },
    async assign(input) {
    ...
    async submit(id,input) {
      ...
      await runTransaction(fs,async tx=>{
        ...
        const patch=submissionPatch(s.data(),a,input);
        tx.update(ref,patch);
    ...
    async review(id,input) {
      ...
      await runTransaction(fs,async tx=>{
        const s=await tx.get(ref);unchanged(a);requireValue(s.exists(),'کام دستیاب نہیں رہا۔');
        tx.update(ref,reviewPatch(s.data(),a,input));
      });
    },
