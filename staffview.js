...
import {
  ...
  monthSummary, openRecord, workMinutes, parseTime, checkoutDue
} from './core.js';
...
export function createStaffView({ data, rerender, logout, checkUpdate }) {
  ...
  function actionCard() {
    ...
    if (open) {
      const started = open.checkInTs || Date.now(), mins = Math.max(0, Math.round((Date.now() - started) / 60000));
      return `<section class="punch is-in"><p class="punch-state">${icon('check', 18)} Aap kaam par hain</p>
        ...
        <p class="punch-sub">${open.checkInTs ? hm(mins) + ' ho gaye' : ''}${sch.shiftEnd ? ' &nbsp;|&nbsp; Chutti ' + fmtTime(sch.shiftEnd) : ''}</p>
    ...
    if (todayRow?.checkIn) {
      return `<section class="punch is-done"><p class="punch-state">${icon('check', 18)} Aaj ki hazri mukammal</p>
        ...
        <p class="punch-sub">${workMinutes(todayRow) != null ? hm(workMinutes(todayRow)) + ' kaam kiya' : ''}</p></section>`;
    }
    ...
    return `<section class="punch"><p class="punch-state">${esc(dateLabel(today))}</p>
      <p class="punch-big">Duty ${fmtTime(sch.shiftStart)}</p>
  ...
  function salaryTab() {
    ...
    return `<div class="toolbar"><h1 class="page-title">Meri salary</h1><span class="stamp ${c.frozen ? 'st-present' : 'st-waiting'}">${c.frozen ? 'Final' : 'Andaza'}</span></div>
      ...
      <section class="panel pad"><table class="calc"><tbody>
        ...
        ${line(`Aam ghante <small>${hm(c.normalMin)}</small>`, money(c.normalSalary))}
